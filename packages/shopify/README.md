...
A Model Context Protocol (MCP) server that provides AI assistants (Claude, Cursor, etc.) with a comprehensive, read-only interface to the Shopify Admin API. **Supports both static access tokens and OAuth2 authentication.**
...
- **Comprehensive Coverage:** Read-only access to Orders, Products, Customers, and Shop info.
