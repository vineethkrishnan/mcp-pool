...
A Model Context Protocol (MCP) server that provides AI assistants (Claude, Cursor, etc.) with a comprehensive, read-only interface to the Vercel API. **Supports team-scoped access.**
...
- **Comprehensive Coverage:** Read-only access to Projects, Deployments, and Build Logs.
