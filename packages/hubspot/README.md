...
A Model Context Protocol (MCP) server that provides AI assistants (Claude, Cursor, etc.) with a comprehensive, read-only interface to the HubSpot CRM API. **Supports both static access tokens and OAuth2 authentication.**
...
- **Comprehensive Coverage:** Read-only access to Contacts, Deals, and Companies.
