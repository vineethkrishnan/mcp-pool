...
A Model Context Protocol (MCP) server that provides AI assistants (Claude, Cursor, etc.) with a comprehensive, read-only interface to the Linear API via GraphQL. **Supports both static API keys and OAuth2 authentication.**
...
- **Comprehensive Coverage:** Read-only access to Issues, Projects, and Teams.
